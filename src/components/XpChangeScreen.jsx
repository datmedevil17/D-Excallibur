    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 z-51">
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
        ...
        <div className="space-y-2 text-gray-600">
          <p>
            <span className="font-semibold">Kills:</span> {kill}
          </p>
          <p>
            <span className="font-semibold">Deaths:</span> {death}
          </p>
          <p>
            <span className="font-semibold">XP Change:</span> {newXP}
          </p>
          <p>
          ...
          <p>
            <span className="font-semibold">Old Token:</span> {token}
          </p>
          <p>
            <span className="font-semibold">New Token:</span>{" "}
            {Number(token) + Math.max(Number(newXP), 0)}
          </p>
          <p>
            <span className="font-semibold">Old Rank:</span>{" "}
            {getRank(Number(xp))}
          </p>
          <p>
            <span className="font-semibold">New Rank:</span>{" "}
            {getRank(Number(xp) + Number(newXP))}
          </p>
        </div>
        <button
          onClick={onBack}
